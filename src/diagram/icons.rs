...
/// Generated by `data/tools/build_azure2_manifest.py`; see its docstring.
const AZURE2_STENCILS: &str = include_str!("../../data/azure2_stencils.toml");

/// Every stencil draw.io ships, mapped to the aspect it ships at.
///
/// draw.io renders a path it cannot resolve as a broken-image placeholder
/// rather than failing, so a wrong category is invisible until someone opens
/// the workbook. Checking against this turns that into a test failure.
fn stencils() -> &'static BTreeMap<String, f64> {
    static STENCILS: OnceLock<BTreeMap<String, f64>> = OnceLock::new();
    STENCILS.get_or_init(|| {
        #[derive(serde::Deserialize)]
        struct Raw {
            stencils: BTreeMap<String, f64>,
        }
        let raw: Raw = toml::from_str(AZURE2_STENCILS)
            .expect("embedded azure2_stencils.toml is valid; guaranteed by unit test");
        raw.stencils
    })
}

/// Aspect a stencil is drawn at, or 1.0 for a path not in the manifest.
///
/// The fallback keeps a mapping typo cosmetic at runtime — the test is what
/// makes it loud — rather than turning it into a panic in a user's export.
fn stencil_aspect(stencil: &str) -> f64 {
    stencils().get(stencil).copied().unwrap_or(1.0)
}
...
pub fn header_style(kind: &NodeKind) -> Option<(String, f64)> {
    let stencil = match kind {
        NodeKind::Tenant => "general/Management_Groups",
        NodeKind::Subscription => "general/Subscriptions",
        NodeKind::ResourceGroup => "general/Resource_Groups",
        NodeKind::Vnet => "networking/Virtual_Networks",
        NodeKind::Subnet => "networking/Subnet",
        NodeKind::Zone => "general/All_Resources",
        // A resource is drawn as its own glyph; it has no band to sit in.
        NodeKind::Resource { .. } => return None,
    };
    // Stencils are not square — a virtual network is 18.0 x 10.8 — and the
    // ratio comes from the generated manifest rather than being copied here,
    // so a draw.io upgrade that reshapes one is a re-run, not a hunt.
    let aspect = stencil_aspect(stencil);
...
mod tests {
    ...
    /// Every draw.io stencil path the emitter can produce must exist in
    /// draw.io's library. A path it cannot resolve renders as a broken-image
    /// placeholder rather than failing, so `microsoft.web/staticsites` shipped
    /// pointing at `app_services` when the stencil lives under `preview`, and
    /// it took someone opening a workbook to notice.
    #[test]
    fn unit_every_mapped_stencil_exists_in_the_drawio_library() {
        let library = stencils();
        let missing: Vec<String> = ICONS
            .iter()
            .map(|(_, category, name)| format!("{category}/{name}"))
            .chain(std::iter::once(format!("{}/{}", GENERIC.0, GENERIC.1)))
            .filter(|stencil| !library.contains_key(stencil))
            .collect();

        assert!(
            missing.is_empty(),
            "not in data/azure2_stencils.toml: {missing:?}"
        );
    }

    /// The container stencils are picked by an exhaustive match rather than
    /// from the mapping table, so they need the same check.
    #[test]
    fn unit_every_container_stencil_exists_in_the_drawio_library() {
        for kind in [
            NodeKind::Tenant,
            ...
            let (style, aspect) = header_style(&kind).expect("a container stencil");
            let stencil = style
                .split("img/lib/azure2/")
                .nth(1)
                .and_then(|rest| rest.split(".svg").next())
                .expect("a stencil path");

            assert!(
                stencils().contains_key(stencil),
                "{stencil} is not in data/azure2_stencils.toml"
            );
            // A missing entry falls back to 1.0, which would silently square a
            // landscape glyph — the defect this manifest exists to prevent.
            assert_eq!(aspect, stencils()[stencil]);
        }
    }

    #[test]
    fn unit_the_stencil_manifest_parses_and_is_not_empty() {
        assert!(stencils().len() > 500, "manifest looks truncated");
        assert!(
            stencils().values().all(|&a| a > 0.0),
            "a non-positive aspect"
        );
    }
}
