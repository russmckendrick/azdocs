...
/// Stand-in group name for a resource that carries no resource group.
///
/// Capitalised because it is displayed: it reaches the relationship map as a
/// group name and the record view already labelled such resources this way.
/// Casing is safe here — the join key lowercases both halves and the synthetic
/// id lowercases the name.
pub const SUBSCRIPTION_SCOPE: &str = "Subscription scope";
...
pub fn bucket_resources<'a>(
    ...
    for resource in resources {
        let name = resource
            .resource_group
            .clone()
            .unwrap_or_else(|| SUBSCRIPTION_SCOPE.to_owned());
...
mod tests {
    ...
    fn unit_places_resources_in_their_declared_group_when_a_row_exists() {
        ...
        let (buckets, by_resource) = bucket_resources(&groups, &resources, &[]);
    ...
    fn unit_matches_across_casing_when_arg_disagrees_with_itself() {
        ...
        let (buckets, _) = bucket_resources(&groups, &resources, &[]);
    ...
    fn unit_synthesises_a_group_when_the_snapshot_has_no_row_for_it() {
        ...
        let (buckets, _) = bucket_resources(&[], &resources, &[]);
    ...
    fn unit_gathers_group_less_resources_under_subscription_scope() {
        ...
        assert_eq!(buckets.len(), 1);
        assert_eq!(buckets[0].name, SUBSCRIPTION_SCOPE);
    ...
    fn unit_drops_groups_outside_the_requested_subscriptions() {
        ...
        let (buckets, _) = bucket_resources(&groups, &resources, &["s1".to_owned()]);
    ...
    fn unit_keeps_declared_groups_that_hold_no_resources() {
        ...
        let (buckets, _) = bucket_resources(&groups, &[], &[]);
